...
export class CartAdminService {
  ...
  constructor() {
    ...
    this.cart = new BehaviorSubject<any>(
      JSON.parse(localStorage.getItem('ordenAdmin'))
    );
  ...
  saveCart(): void {
    localStorage.setItem('ordenAdmin', JSON.stringify(this.cart.getValue()));
  }
  ...
  addToCart(producto: any, idTable: any) {
    ...
    if (listCart) {
      ...
      let objIndex = listCart.findIndex(
        (obj) => obj.idItem == newItem.idItem && obj.idTable == newItem.idTable
      );
